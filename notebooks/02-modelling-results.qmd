...
```{r}
model_performance(fit_finer_i)

model_parameters(fit_finer_i)
```

```{r}
model_performance(fit_finer_e)

model_parameters(fit_finer_e)
```
...
Anova(fit_finer_i, type = 3)
Anova(fit_finer_e, type = 3)
